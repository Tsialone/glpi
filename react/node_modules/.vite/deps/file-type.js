...
function tarHeaderChecksumMatches(arrayBuffer, offset = 0) {
	const readSum = Number.parseInt(new StringType(6).get(arrayBuffer, 148).replace(/* @__PURE__ */ new RegExp("\\0.*$", "v"), "").trim(), 8);
...
var recoverableZipErrorCodes = new Set([
	"Z_BUF_ERROR",
	"Z_DATA_ERROR",
	"ERR_INVALID_STATE"
]);
	...
	try {
		await new ZipHandler(tokenizer).unzip((zipHeader) => {
			...
			switch (zipHeader.filename) {
				...
				default:
					if ((/* @__PURE__ */ new RegExp("classes\\d*\\.dex", "v")).test(zipHeader.filename)) {
						fileType = {
							ext: "apk",
	...
	async function readChildren(children) {
		...
		while (children > 0) {
			...
			if (element.id === 17026) {
				...
				return (await tokenizer.readToken(new StringType(documentTypeLength))).replaceAll(/* @__PURE__ */ new RegExp("\\0.*$", "gv"), "");
			}
...
var FileTypeParser = class FileTypeParser {
	...
	detectConfident = async (tokenizer) => {
		...
		if (this.checkString("AC")) {
			...
			if ((/* @__PURE__ */ new RegExp("^\\d+$", "v")).test(version) && version >= 1e3 && version <= 1050) return {
				ext: "dwg",
				mime: "image/vnd.dwg"
			};
		}
...
//#endregion
export { FileTypeParser, fileTypeFromBlob, fileTypeFromBuffer, fileTypeFromFile, fileTypeFromStream, fileTypeFromTokenizer, fileTypeStream, normalizeSampleSize, reasonableDetectionSizeInBytes, supportedExtensions, supportedMimeTypes };
